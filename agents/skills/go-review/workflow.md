...
Combined staged + unstaged tracked `.go` vs `{{target_branch}}`:

```bash
git diff {{target_branch}} -- '*.go'
```

One diff stream covers every tracked `.go` that differs from the target (staged, unstaged, or mixed).
...
**Contract/deployment/dependency manifest.** Also list non-Go contract, deployment, dependency, and tooling files that differ vs `{{target_branch}}` so any interested agent can iterate them. Run:
...
If no `.go` files are in the diff and `contract-files.txt` is empty, stop and report: "No uncommitted Go, dependency, contract, or deployment files found relative to `{{target_branch}}`."

### Step 1.4 — Prefetch changed Go files
...
### Step 1.7 — Build review-context.json and route agents
...
Non-Go contract/deployment/dependency files changed in this review are listed in {{tmp_dir}}/contract-files.txt (one path per line, relative to {{repo_root}}). Use it if your checklist cares about `.proto`, SQL migrations, Helm/K8s manifests, OpenAPI, CI config, `go.mod`, `go.sum`, Dockerfiles, or codegen config.
...
Set:
- `scope_status = "in_scope"` when the issue is caused by, exposed by, or required to safely commit this local change.
- `scope_status = "out_of_scope"` when the issue is pre-existing or unrelated to the reviewed change.
- `scope_reason` to one concise sentence explaining the causal link or why the finding is independent from this local change.
...
- **What This Change Does** — 2–5 sentences derived from the diff + data-flow summaries.
...
- **Verification** — list commands actually run by the orchestrator (usually none on local flow; state `Static review only; no commands executed by the orchestrator.` when so). List every `requires_verification: true` finding in "Could not verify". List `--only` exclusions and router-skipped selected agents under "Gaps".
