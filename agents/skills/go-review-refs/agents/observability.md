...
For every `.go` file in the diff — and for any changed Kubernetes/Helm/CI manifests in the MR — check ALL of the following.
...
Read `go-review-refs/context-rules/observability.md` before starting analysis. Follow its triggers to load the project's logger, metric registry, tracing middleware, and deployment manifests when needed.
