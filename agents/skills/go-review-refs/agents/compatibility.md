...
- [ ] Field renamed **and** renumbered in the same MR — hides the renumber
...
- [ ] Change requires a specific **rollout order** across services but the MR does not call it out (e.g. deploy producer before consumer, migrate schema before deploying binary)
...
- Do NOT suggest speculative API v-next work unrelated to this MR.
...
- Call out **cross-MR** coordination explicitly when this MR's safety depends on another MR shipping first or last.
...
**Code snippets (JSON):** Default **mode A**. If one diff line/hunk or code at cited `file`/`line` in the loaded file suffices for the issue → **MUST** non-empty `code_before`/`code_after`; **forbidden:** mode B to dodge pasting diff. Waiver: `code_snippet_unavailable` true + English `code_absence_note` (≥20 chars) + empty `code_before`/`code_after` **only** when no honest single-site excerpt (cross-cutting; policy-only; artifact absent from diff — e.g. "migration lands in stacked MR !N"). Contract `go-review-refs/agent-output-schema.json`; modes A/B `go-review-refs/report-format.md`.
...
- If no findings when the MR touches `.proto`, SQL migrations, public HTTP routes, Redis keys, or published event types, re-examine the most-exposed contract once more. If still no findings, return empty `findings` array — do NOT fabricate.
...
Read `go-review-refs/context-rules/compatibility.md` before starting analysis. Compatibility almost always requires pulling in the **other side** of a contract (producers when consumers changed, client SDKs when servers changed, old SQL migration files when schema changed, prior proto revisions when fields changed).
