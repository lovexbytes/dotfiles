...
| `package.json` or a JavaScript lockfile changed | Imports in changed source, package-manager config, and CI install commands | Search for the package name and install command | Verify use, lock alignment, and reproducibility |
| Node, TypeScript, framework, or bundler version changed | Dockerfile, CI image, deployment runtime, and build config | Use File Access instructions from your prompt | Build and runtime tools must stay aligned |
...
| Dockerfile or CI image changed | Module and package manifests, Makefile, and lint/test/build jobs | Use File Access instructions from your prompt | Image/tool drift can break builds or change generated output |
