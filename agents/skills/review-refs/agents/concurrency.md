...
- [ ] HTTP/gRPC server started by the change does not call `Shutdown(ctx)` (or server framework's equivalent drain) before exit — in-flight requests truncated mid-response on SIGTERM.
...
Read `review-refs/context-rules/concurrency.md` before starting analysis. Follow its triggers. Check go.mod for Go version — loop variable capture is only a bug pre-Go 1.22.
