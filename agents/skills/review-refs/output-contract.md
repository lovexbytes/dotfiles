...
Return JSON matching `review-refs/agent-output-schema.json`.
...
- Optional `scope_status` / `scope_reason` may be omitted by sub-agents. Final scope classification is added by the orchestrator after deduplication using `review-refs/scope-validation.md`.
